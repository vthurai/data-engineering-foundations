...
We will use a method of sorting called merge sort.
...
Step 4: now we need to merge the two lists into one sorted list. To do this:
  Step 4a: compare the first element of both left and right sublists. You should keep track of their indexes to make things easier.
  Step 4b: pick the smaller element and place it in the correct position in the original list.
  Step 4c: move on to the next index. Compare elements again, and continue until both left and right lists are exhausted. 
  Step 4d: if there are any elements remaining that were not compared, add them to the original list.
Step 5: The list should now be sorted.
